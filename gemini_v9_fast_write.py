...
def write_to_plc(plc, data, type_mapping):
    ...
    try:
        ...
        ACK_TIMEOUT_SECONDS = 5 # Reverted to 5 seconds. If PLC takes longer, Python will report timeout.
        ...
        if request_ack is not None and request_ack > 0:
            try:
                ...
                ack_message = {
                    ...
                    # Removed Python_ACK_Timestamp and Python_ACK_ID for backend compatibility
                }
                print(f"DEBUG: About to send ACK message to Azure IoT Hub.") # Updated debug print
...
# In your process_queue function:
def process_queue(plc):
    ...
    while True:
        ...
        time.sleep(0.5) # Keep this delay for stability

        # >>> Start timing for the entire queue processing for this message <<<
        queue_process_start_time = time.time()
        ...
        queue_process_end_time = time.time()
        print(f"⏱ Total time for message from queue to PLC write completion: {queue_process_end_time - queue_process_start_time:.4f} seconds.")
