    ...
    try:
        ...
        for node in _WRITE_NODES_LOCAL:
            ...
            if mapped_key_from_data and mapped_key_from_data in data:
                ...
                write_commands.append((var_name, value_to_write, var_type))
            else:
        ...
        if write_commands:
            try:
                # Perform bulk write for Request_Data variables
                plc.write_by_name_list(write_commands)
                for var_name, value_to_write, _ in write_commands:
                    print(f"✅ Successfully written '{value_to_write}' to '{var_name}' (Bulk)")
            except pyads.pyads_ex.ADSError as ads_e:
                print(f"❌ ADSError during bulk writing Request_Data: {ads_e} (ADS Error Code: {ads_e.error_code})")
                traceback.print_exc()
                raise
            except Exception as e:
                print(f"❌ General Error during bulk writing Request_Data: {e}")
                traceback.print_exc()
                raise
        ...
        try:
            ...
            # Reduced sleep, will rely on polling for ack
        ...
        # Prepare for bulk read of acknowledgment related variables
        ack_read_commands = [
            (".PLC_To_Server.Request_Ack", pyads.PLCTYPE_BYTE),
            (".Server_To_PLC.Request_Data.Token_No", pyads.PLCTYPE_INT),
            (".PLC_To_Server.System_Code_No", pyads.PLCTYPE_WORD),
            (".Server_To_PLC.Request_Data.Request_Type", pyads.PLCTYPE_BYTE) # For read_request_type
        ]
        ...
        while request_ack is None or request_ack == 0:
            if time.time() - ack_timeout_start > ACK_TIMEOUT_SECONDS:
                print(f"❌ Acknowledgment timeout after {ACK_TIMEOUT_SECONDS} seconds.")
                break # Exit loop if timeout reached

            try:
                ack_results = plc.read_by_name_list(ack_read_commands)
                
                # Unpack results
                request_ack, ack_error_ack = ack_results[0]
                token_no_from_plc, ack_error_token = ack_results[1]
                system_code_no, ack_error_syscode = ack_results[2]
                raw_request_type_val, ack_error_reqtype = ack_results[3]

                if ack_error_ack == 0:
                    # print(f"DEBUG: Polling Request_Ack: {request_ack}") # Uncomment for verbose polling debug
                    pass # Keep polling if 0
                else:
                    print(f"❌ Error reading Request_Ack during polling: ADS Error {ack_error_ack}")
                    break # Break on error

                if ack_error_token != 0: print(f"❌ Error reading Token_No during polling: ADS Error {ack_error_token}")
                if ack_error_syscode != 0: print(f"❌ Error reading System_Code_No during polling: ADS Error {ack_error_syscode}")
                if ack_error_reqtype != 0: print(f"❌ Error reading Request_Type during polling: ADS Error {ack_error_reqtype}")

                if request_ack is not None and request_ack > 0:
                    print(f"📥 Request_Ack received from PLC: {request_ack}")
                ...
                print(f"❌ ADSError during acknowledgment polling (bulk read): {ads_e} (ADS Error Code: {ads_e.error_code})")
                traceback.print_exc()
                break # Break on critical ADS error
            except Exception as e:
                print(f"❌ General Error during acknowledgment polling: {e}")
                traceback.print_exc()
        ...
        if request_ack is not None and request_ack > 0:
            try:
                ...
                time.sleep(0.01) # Reduced from 0.05s, minimal delay after reset
        ...
        print("👍 Beckhoff write operation cycle completed.")
