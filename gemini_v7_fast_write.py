...
def write_to_plc(plc, data, type_mapping):
    ...
    try:
        ...
        ACK_TIMEOUT_SECONDS = 15 # Increased timeout to 15 seconds to match observed PLC response time
        ...
        if request_ack is not None and request_ack > 0:
            try:
                ...
                ack_message = {
                    ...
                    "Python_ACK_Timestamp": datetime.now().isoformat(), # Add Python script's timestamp
                    "Python_ACK_ID": str(uuid.uuid4()) # Add a unique ID for correlation
                }
